...
class AdaptiveAnimation(Scene):
    ...
    def move_kernel_and_apply_sobel(self, kernel_visual, canny_image_manim, gray_image, i, j, step_x, step_y,):
        threshold = int(os.getenv('ADAPTIVE_THRESHOLD'))
        constant = int(os.getenv('CONSTANT'))
        ...
        thresh_adaptive = cv2.adaptiveThreshold(gray_image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                        cv2.THRESH_BINARY, threshold, constant)


        # Extract the relevant region from the filtered image
        thresh_adaptive_region = thresh_adaptive[start_y:end_y, start_x:end_x]

        # Update the blurred image only in the kernel region
        gray_image[start_y:end_y, start_x:end_x] = np.where(mask == 1, thresh_adaptive_region, gray_image[start_y:end_y, start_x:end_x])

        # Save the progressively filtered image
        treshold_adaptive_image_path = 'treshold_adaptive_image.png'
        cv2.imwrite(treshold_adaptive_image_path, gray_image)

        # Update the right-side image with the progressively filtered region
        canny_image_updated = ImageMobject(treshold_adaptive_image_path)
