...
class CannyFilterAnimation(Scene):
    def construct(self):
        ...
        canny_image_manim = ImageMobject(blurred_image_path)
        ...
        for j in range(int(img_height // step_y)):
            for i in range(int(img_width // step_x)):
                ...
                if j % 2 == 0:  # Even row: left to right
                    self.move_kernel_and_apply_sobel(kernel_visual, canny_image_manim, blurred_image, i, j, step_x, step_y)
                else:  # Odd row: right to left
                    self.move_kernel_and_apply_sobel(kernel_visual, canny_image_manim, blurred_image, int(img_width // step_x) - 1 - i, j, step_x, step_y)

        self.wait(2)

    def move_kernel_and_apply_sobel(self, kernel_visual, canny_image_manim, blurred_image, i, j, step_x, step_y):
        ...
        blurred_image[start_y:end_y, start_x:end_x] = np.where(mask == 1, canny_filtered, blurred_image[start_y:end_y, start_x:end_x])

  
        # Save the progressively filtered image
        canny_image_path='canny.png'
        cv2.imwrite(canny_image_path, blurred_image)

        # Update the right-side image with the progressively filtered region
        canny_image_updated = ImageMobject(canny_image_path)
