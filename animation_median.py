...
class MedianAnimation(Scene):
    def construct(self):
        ...
        median_image_manim = ImageMobject(image_path)
        median_image_manim.set_width(6)  # Set new width
        median_image_manim.set_height(4.5)  # Set new height
        median_image_manim.shift(RIGHT * 4)

        self.clear()

        # Show the original and processed images side by side
        self.play(FadeIn(original_image_manim), FadeIn(median_image_manim))

        # Create a visual representation of the Sobel kernel
        kernel_visual = Square(side_length=1.5, color=YELLOW)
        kernel_visual.move_to((median_image_manim.get_corner(UL)) + RIGHT * 0.4 + DOWN * 0.4)
        ...
        for j in range(int(img_height // step_y)):
            for i in range(int(img_width // step_x)):
                ...
                if j % 2 == 0:  # Even row: left to right
                    self.move_kernel_and_apply_sobel(kernel_visual, median_image_manim, gray_image, i, j, step_x, step_y)
                else:  # Odd row: right to left
                    self.move_kernel_and_apply_sobel(kernel_visual, median_image_manim, gray_image, int(img_width // step_x) - 1 - i, j, step_x, step_y)

        self.wait(2)

    def move_kernel_and_apply_sobel(self, kernel_visual, median_image_manim, gray_image, i, j, step_x, step_y,):
        ...
        self.play(kernel_visual.animate.move_to((median_image_manim.get_corner(UL) + RIGHT * 0.75 + DOWN * 0.75) + RIGHT * new_x + DOWN * new_y), run_time=0.5)
        ...
        median_updated = cv2.medianBlur(gray_image, ksize) 
        # Aplikuj Gaussov filter na farebný obrázok


        # Extrahuj relevantné časti
        median_updated_region = median_updated [start_y:end_y, start_x:end_x]
        ...
        gray_image[start_y:end_y, start_x:end_x] = np.where(mask_3ch == 1, median_updated_region, original_region)

        # Save the progressively filtered image
        median_updated_image_path = 'median_updated _image.png'
        cv2.imwrite(median_updated_image_path, gray_image)

        # Update the right-side image with the progressively filtered region
        canny_image_updated = ImageMobject(median_updated_image_path)
        canny_image_updated.set_width(6).shift(RIGHT * 4)

        # Show the filtered image transition
        self.play(Transform(median_image_manim, canny_image_updated), run_time=0.5)
